...
class CustomRecentsPanelActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        android.util.Log.d("CustomRecents", "CustomRecentsPanelActivity onCreate called")
    ...
    private fun loadRecentTasks() {
        bgExecutor.execute {
            try {
                ...
                val allTasks = am.getRecentTasks(20, ActivityManager.RECENT_IGNORE_UNAVAILABLE)
                android.util.Log.d("CustomRecents", "Total tasks from system: ${allTasks.size}")
                
                val recentTasks = allTasks.filter {
                    it.baseIntent?.component?.packageName != packageName
                }
                
                android.util.Log.d("CustomRecents", "Filtered tasks (excluding launcher): ${recentTasks.size}")
                
                mainHandler.post { 
                    adapter.submitList(recentTasks)
                    
                    // If no tasks, show a message or auto-close
                    if (recentTasks.isEmpty()) {
                        android.util.Log.w("CustomRecents", "No recent tasks available")
                        // Optionally auto-close if no tasks
                        // finish()
                    }
                }
            } catch (e: Exception) {
                android.util.Log.e("CustomRecents", "Error loading recent tasks", e)
                mainHandler.post { 
