...
fun PomodoroScreen(onExit: () -> Unit) {
        ...
        Box(
                modifier = Modifier.fillMaxSize().background(POM_BG),
        ) {
                when (phase) {
                        PomodoroPhase.SETUP ->
                                SetupPanel(
                                        ...
                                        onStart = {
                                                totalSeconds =
                                                        selectedHour * 3600 + selectedMin * 60
                                                elapsedSeconds = 0
...
// ── Card-style picker with bigger buttons + long-press repeat ────────────────
@Composable
...
/** A box that fires [onClick] on tap, and repeats it while the finger is held down. */
@Composable
        ...
        Box(
                modifier =
                        modifier.pointerInput(Unit) {
                                detectTapGestures(
                                        onPress = {
                                                currentOnClick() // fire immediately
                                                coroutineScope {
                                                        ...
                                                        // tryAwaitRelease suspends until finger
                                                        // lifts,
                                                        // then coroutineScope ends → repeatJob is
                                                        // cancelled
                                                        tryAwaitRelease()
                                                        repeatJob.cancel()
                                                }
...
private fun CancelButton(onClick: () -> Unit) {
        TextButton(onClick = onClick) { Text("Cancel", color = POM_DIM40, fontSize = 14.sp) }
}
...
private fun ClockPanel(
        ...
        val remStr =
                if (remH > 0) "%d:%02d:%02d".format(remH, remM, remS)
                ...
                if (isLandscape) {
                        Row(
                                ...
                                Column(
                                        ...
                                        Text(
                                                remStr,
                                                color = if (isDone) POM_ACCENT else POM_DIM40,
                                                fontSize = 28.sp,
                                                fontWeight = FontWeight.Light,
                                                letterSpacing = 2.sp
                                        )
