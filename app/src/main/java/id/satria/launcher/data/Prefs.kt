...
class Prefs(private val context: Context) {
    ...
    val hiddenPackages =
            ds.data.map {
                decode(it[PrefKeys.HIDDEN_PACKAGES], emptyList()) { s ->
                    json.decodeFromString<List<String>>(s)
                }
            }
    val dockPackages =
            ds.data.map {
                decode(it[PrefKeys.DOCK_PACKAGES], emptyList()) { s ->
                    json.decodeFromString<List<String>>(s)
                }
            }
    val todos =
            ds.data.map {
                decode(it[PrefKeys.TODOS], emptyList()) { s ->
                    json.decodeFromString<List<TodoItem>>(s)
                }
            }
    val countdowns =
            ds.data.map {
                decode(it[PrefKeys.COUNTDOWNS], emptyList()) { s ->
                    json.decodeFromString<List<CountdownItem>>(s)
                }
            }
    val weatherLocations =
            ds.data.map {
                decode(it[PrefKeys.WEATHER_LOCATIONS], emptyList()) { s ->
                    json.decodeFromString<List<String>>(s)
                }
            }
    val prayerCities =
            ds.data.map {
                decode(it[PrefKeys.PRAYER_CITIES], emptyList()) { s ->
                    json.decodeFromString<List<String>>(s)
                }
            }
    ...
    val habits =
            ds.data.map {
                decode(it[PrefKeys.HABITS], emptyList()) { s ->
                    json.decodeFromString<List<HabitItem>>(s)
                }
            }
    val moneyWallets =
            ds.data.map {
                decode(it[PrefKeys.MONEY_WALLETS], emptyList()) { s ->
                    json.decodeFromString<List<MoneyWallet>>(s)
                }
            }
    val moneyTransactions =
            ds.data.map {
                decode(it[PrefKeys.MONEY_TRANSACTIONS], emptyList()) { s ->
                    json.decodeFromString<List<MoneyTransaction>>(s)
                }
            }
    ...
    val enableCustomRecents =
            ds.data.map { it[PrefKeys.ENABLE_CUSTOM_RECENTS] ?: true }.distinctUntilChanged()

    // ── Grid layout ───────────────────────────────────────────────────────
    val gridCols =
            ds.data.map { it[PrefKeys.GRID_COLS] ?: DEFAULT_GRID_COLS }.distinctUntilChanged()
    val gridRows =
            ds.data.map { it[PrefKeys.GRID_ROWS] ?: DEFAULT_GRID_ROWS }.distinctUntilChanged()
    ...
    suspend fun setHiddenPackages(v: List<String>) =
            ds.edit { it[PrefKeys.HIDDEN_PACKAGES] = json.encodeToString(v) }
    suspend fun setDockPackages(v: List<String>) =
            ds.edit { it[PrefKeys.DOCK_PACKAGES] = json.encodeToString(v) }
    suspend fun setTodos(v: List<TodoItem>) =
            ds.edit { it[PrefKeys.TODOS] = json.encodeToString(v) }
    suspend fun setCountdowns(v: List<CountdownItem>) =
            ds.edit { it[PrefKeys.COUNTDOWNS] = json.encodeToString(v) }
    suspend fun setWeatherLocations(v: List<String>) =
            ds.edit { it[PrefKeys.WEATHER_LOCATIONS] = json.encodeToString(v) }
    suspend fun setPrayerCities(v: List<String>) =
            ds.edit { it[PrefKeys.PRAYER_CITIES] = json.encodeToString(v) }
    suspend fun setPrayerCache(v: String) = ds.edit { it[PrefKeys.PRAYER_CACHE] = v }
    suspend fun setHabits(v: List<HabitItem>) =
            ds.edit { it[PrefKeys.HABITS] = json.encodeToString(v) }
    suspend fun setMoneyWallets(v: List<MoneyWallet>) =
            ds.edit { it[PrefKeys.MONEY_WALLETS] = json.encodeToString(v) }
    suspend fun setMoneyTransactions(v: List<MoneyTransaction>) =
            ds.edit { it[PrefKeys.MONEY_TRANSACTIONS] = json.encodeToString(v) }
    ...
    suspend fun setEnableCustomRecents(v: Boolean) =
            ds.edit { it[PrefKeys.ENABLE_CUSTOM_RECENTS] = v }
}
