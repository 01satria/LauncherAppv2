                        ...
                        AnimatedContent(
                            ...
                            when (tool) {
                                "weather" ->
                                        WeatherTool(
                                                savedLocations =
                                                        vm.weatherLocations.collectAsState().value,
                                ...
                                "moneymanager" ->
                                        MoneyTool(
                                                ...
                                                transactions =
                                                        vm.moneyTransactions.collectAsState().value,
                                                onAddWallet = { name, emoji, color, currency ->
                                                    vm.addMoneyWallet(name, emoji, color, currency)
                                                },
                                ...
                                "prayer" ->
                                        PrayerTool(
                                                savedCities =
                                                        vm.prayerCities.collectAsState().value,
                                                savedCacheJson =
                                                        vm.prayerCache.collectAsState().value,
                                                onAddCity = { vm.addPrayerCity(it) },
                                                onRemoveCity = { vm.removePrayerCity(it) },
                                                onUpdateCache = { city, json ->
                                                    vm.updatePrayerCache(city, json)
                                                }
                                        )
                                "habits" ->
                                        HabitTool(
                                                habits = habits,
                                                onAdd = { n, e -> vm.addHabit(n, e) },
