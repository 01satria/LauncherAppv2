...
fun MoneyTool(
    ...
    val monthTxs =
            remember(transactions) { transactions.filter { sameMonth(it.date, currentMonthKey) } }
    val totalIncome =
    ...
    val netBalance =
            remember(wallets, transactions) { wallets.sumOf { walletBalance(it, transactions) } }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
            ...
            Column(
                ...
                Row(
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        SmallPill("Export") {
                            exportLauncher.launch("money_backup_${todayDateStr()}.json")
                        }
                        SmallPill("Import") {
                ...
                tabs.forEach { (tab, label) ->
                    ...
                    Box(
                        ...
                        Text(
                                ...
                                color = if (sel) acc else SatriaColors.TextTertiary
                        )
                    }
                }
            }
            ...
            Box(Modifier.weight(1f)) {
                when (activeTab) {
                    ...
                    MoneyTab.TRANSACTIONS ->
                            TransactionsTab(wallets, transactions, onDeleteTransaction)
        ...
        // Sheets
        if (showAddTx)
                AddTransactionSheet(wallets, wallets.firstOrNull()?.id, { showAddTx = false }) {
                        wId,
                        type,
                        amt,
                        cat,
                    ...
                    items(wallets) { w ->
                        ...
                        Box(
                            ...
                            Column {
                                ...
                                Text(
                                        formatAmountShort(balance, w.currency),
                                        color =
                                                if (balance >= 0) Color(0xFF2ECC71)
                        ...
                        Box(
                            ...
                            Box(
                                    Modifier.fillMaxWidth(frac)
                                            .fillMaxHeight()
                                            .clip(RoundedCornerShape(2.dp))
                                            .background(cat?.color ?: SatriaColors.Accent)
                            )
                        }
                    }
                }
...
private fun TransactionsTab(
    ...
    val grouped =
            remember(sorted) { sorted.groupBy { it.date }.entries.sortedByDescending { it.key } }
    if (sorted.isEmpty()) {
            ...
            Box(
                    ...
                    modifier =
                            Modifier.size(38.dp)
                                    .clip(CircleShape)
                                    .background(
                                            (cat?.color ?: Color(0xFF95A5A6)).copy(alpha = 0.15f)
                                    )
            ) { Text(if (tx.type == "transfer") "↔️" else cat?.emoji ?: "💸", fontSize = 16.sp) }
            Column(Modifier.weight(1f)) {
                Text(
                        cat?.label ?: if (tx.type == "transfer") "Transfer" else tx.categoryKey,
                        color = SatriaColors.TextPrimary,
                        fontSize = 13.sp,
                        fontWeight = FontWeight.Medium
                )
            ...
            Box(
                ...
                Column {
                    ...
                    Text(
                            ...
                            color = if (net >= 0) Color(0xFF2ECC71) else Color(0xFFE74C3C),
                            fontSize = 22.sp,
                            fontWeight = FontWeight.Bold
                    )
                    if (total > 0) {
                        ...
                        Row(
                            ...
                            Row(
                                ...
                                Text("Income", color = SatriaColors.TextTertiary, fontSize = 11.sp)
                            }
                            Row(
                                    verticalAlignment = Alignment.CenterVertically,
                                    horizontalArrangement = Arrangement.spacedBy(4.dp)
                                ...
                                Text("Expense", color = SatriaColors.TextTertiary, fontSize = 11.sp)
                            }
                        }
                    }
                }
                        ...
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column(
                                ...
                                byCatExp.take(5).forEach { (k, amt) ->
                                    ...
                                    val pct =
                                            if (totalExpense > 0) (amt / totalExpense * 100).toInt()
                                            else 0
                                    Row(
                                            verticalAlignment = Alignment.CenterVertically,
                                            horizontalArrangement = Arrangement.spacedBy(5.dp)
                                        ...
                                        Box(
                                                Modifier.size(8.dp)
                                                        .clip(CircleShape)
                                                        .background(cat?.color ?: Color(0xFF95A5A6))
                                        )
                                        Text(
                                    ...
                                    Box(
                                        ...
                                        Box(
                                                Modifier.fillMaxWidth(frac)
                                                        .fillMaxHeight()
                                                        .clip(RoundedCornerShape(2.dp))
                                                        .background(cat?.color ?: Color(0xFF2ECC71))
                                        )
                                    }
                                }
                            }
...
private fun WalletsTab(
    ...
    LazyColumn(
        ...
        if (wallets.isEmpty())
                item { EmptyCard("💳", "No wallets yet", "Tap + Add Wallet to get started") }
...
private fun WalletCard(wallet: MoneyWallet, allTxs: List<MoneyTransaction>, onDelete: () -> Unit) {
    ...
    val income =
            remember(txs) {
                txs.filter { it.type == "income" && it.walletId == wallet.id }.sumOf { it.amount }
            }
    val expense =
            remember(txs) {
                txs.filter { it.type == "expense" && it.walletId == wallet.id }.sumOf { it.amount }
            }
    ...
    Box(
        ...
        Column {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                Column {
                    Text("Income", color = SatriaColors.TextTertiary, fontSize = 10.sp)
                    Text(
                ...
                Column {
                    Text("Expense", color = SatriaColors.TextTertiary, fontSize = 10.sp)
                    Text(
                ...
                Column {
                    Text("Transactions", color = SatriaColors.TextTertiary, fontSize = 10.sp)
                    Text(
                            "${txs.size}",
                            color = SatriaColors.TextSecondary,
                            fontSize = 12.sp,
        ...
        Box(
            ...
            Column {
                ...
                Column(
                            ...
                            items(wallets) { w ->
                                ...
                                Box(
                                    ...
                                    Text(
                                            ...
                                            fontSize = 12.sp,
                                            color =
                                                    if (sel) SatriaColors.TextPrimary
                                                    else SatriaColors.TextSecondary
                                    )
                                }
                            }
                    ...
                    OutlinedTextField(
                            ...
                            colors =
                                    OutlinedTextFieldDefaults.colors(
                                            ...
                                            unfocusedBorderColor = SatriaColors.BorderLight,
                                            focusedLabelColor = acc,
                                            unfocusedLabelColor = SatriaColors.TextTertiary,
                            ...
                            items(cats) { c ->
                                ...
                                Box(
                                        ...
                                        modifier =
                                                Modifier.clip(RoundedCornerShape(8.dp))
                                                        .background(
                                                                if (sel) c.color.copy(alpha = 0.2f)
                                                                else SatriaColors.SurfaceMid
                                                        )
                                                        .border(
                                                                if (sel) 1.dp else 0.dp,
                    ...
                    Text("Date", color = SatriaColors.TextTertiary, fontSize = 11.sp)
                    Box(
                            modifier =
                                    Modifier.fillMaxWidth()
                                            .clip(RoundedCornerShape(10.dp))
                    ...
                    Button(
                            onClick = {
                                ...
                                err =
                                        when {
                                            ...
                                            txType != "transfer" && cat == null ->
                                                    "Select a category"
                                            txType == "transfer" && toWId.isEmpty() ->
                            ...
                            modifier = Modifier.fillMaxWidth(),
                            shape = RoundedCornerShape(10.dp),
                            colors = ButtonDefaults.buttonColors(containerColor = accBtn)
                    ) {
                        Text(
                                "Save Transaction",
                                color = Color.White,
                            ...
                            Box(
                                    ...
                                    modifier =
                                            Modifier.size(40.dp)
                                                    .clip(CircleShape)
                                                    .background(
                                                            if (e == emoji) acc.copy(alpha = 0.2f)
                                                            else SatriaColors.SurfaceMid
                                                    )
                                                    .clickable(
                                                            interactionSource =
                            ...
                            Box(
                                    ...
                                    modifier =
                                            Modifier.clip(RoundedCornerShape(8.dp))
                                                    ...
                                                    .border(
                                                            if (sel) 1.dp else 0.dp,
                                                            acc.copy(alpha = if (sel) 0.4f else 0f),
                                                            RoundedCornerShape(8.dp)
                                                    )
                                                    .clickable(
                                                            interactionSource =
                                ...
                                Text(
                                        cur,
                                        fontSize = 12.sp,
                                        color = if (sel) acc else SatriaColors.TextSecondary,
                                        fontWeight =
                                                if (sel) FontWeight.SemiBold else FontWeight.Normal
                                )
                            }
                        }
                    }
